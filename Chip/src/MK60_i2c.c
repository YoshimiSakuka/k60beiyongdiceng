...
void i2c_write_lendata(I2Cn_e i2cn, uint8 SlaveID, uint8 reg, uint8 *ptr, uint8 len)
{
    ...
    for(;len > 0;len--)
    {
      i2c_write_byte(i2cn, *ptr++);                         //发送需要写入的数据
    }
    i2c_Stop(i2cn);
...
//uint8 i2c_read_reg(I2Cn_e i2cn, uint8 SlaveID, uint8 reg, uint8 *ptr, uint8 len)
//{
//
//    //先写入寄存器地址,再读取数据,因此此过程是 I2C 的复合格式,改变数据方向时需要重新启动
//    uint8 result;
...
//    i2c_write_byte(i2cn, ( SlaveID << 1 ) | MWSR);      //发送从机地址和写位
//
//    i2c_write_byte(i2cn, reg);                          //发送从机里的寄存器地址    i2c_RepeatedStart(i2cn);
//    
//    i2c_RepeatedStart(i2cn);                            //复合格式，发送重新启动信号
//
//    i2c_write_byte(i2cn, ( SlaveID << 1) | MRSW );      //发送从机地址和读位
