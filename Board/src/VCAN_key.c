...
void Dial_Switch(void)           //拨码开关
{
    gpio_init(Dial_1, GPO, 0);
    gpio_init(Dial_2, GPO, 0);
    gpio_init(Dial_3, GPO, 0);
    gpio_init(Dial_4, GPO, 0);
    gpio_init(Dial_5, GPO, 0);
    gpio_init(Dial_6, GPO, 0);
 
}
void Common_Key(void)            //普通按键初始化
{
        gpio_init(Key_1, GPI, 0);
        port_init_NoALT(Key_1, PULLUP);
        gpio_init(Key_2, GPI, 0);
        port_init_NoALT(Key_2, PULLUP);
        gpio_init(Key_3, GPI, 0);
        port_init_NoALT(Key_3, PULLUP);
        gpio_init(Key_4, GPI, 0);
        port_init_NoALT(Key_4, PULLUP);
}
        
        
void FiveDir_Key_Init(void)          //五向按键初始化
{
        gpio_init(Key_L, GPI, 0);
        port_init_NoALT(Key_L, PULLUP);
        gpio_init(Key_R, GPI, 0);
        port_init_NoALT(Key_R, PULLUP);
        gpio_init(Key_U, GPI, 0);
        port_init_NoALT(Key_U, PULLUP);
        gpio_init(Key_D, GPI, 0);
        port_init_NoALT(Key_D, PULLUP);
        gpio_init(Key_M, GPI, 0);
        port_init_NoALT(Key_M, PULLUP);
