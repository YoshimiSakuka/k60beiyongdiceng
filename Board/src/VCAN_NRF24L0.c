...
uint8 TX_ADDRESS[5] = {0x62, 0xA7, 0x31, 0xDB, 0x11};   // 定义一个静态发送地址
uint8 RX_ADDRESS[5] = {0x62, 0xA7, 0x31, 0xDB, 0x11};

#define CHANAL          25                              // 频道选择 位于 0~255
...
uint8 NRF24L01_RXDATA[DATA_PACKET];//nrf24l01接收到的数据
uint8 NRF24L01_TXDATA[DATA_PACKET];//nrf24l01需要发送的数据
...
{
    //配置NRF管脚复用
    spi_init(NRF_SPI, NRF_CS, MASTER,10*1000*1000);                     //初始化SPI,主机模式
...
int NRF_Send_Data(void)
{
	...
	NRF24L01_TXDATA[1]  = BYTE1(i);
	NRF24L01_TXDATA[2]  = BYTE0(i);
	NRF24L01_TXDATA[3]  = BYTE1(i);//实际速度
	NRF24L01_TXDATA[4]  = BYTE0(i);
	NRF24L01_TXDATA[5]  = BYTE1(i);  //速度误差
	NRF24L01_TXDATA[6]  = BYTE0(i);
	NRF24L01_TXDATA[7]  = BYTE1(i);   //输出积分
	NRF24L01_TXDATA[8]  = BYTE0(i);
	NRF24L01_TXDATA[9]  = BYTE1(i); //误差积分
	NRF24L01_TXDATA[10] = BYTE0(i);
	NRF24L01_TXDATA[11] = BYTE1(i);//速度环输出
	NRF24L01_TXDATA[12] = BYTE0(i);
	NRF24L01_TXDATA[13] = BYTE1(i);
	NRF24L01_TXDATA[14] = BYTE0(i);
	NRF24L01_TXDATA[15] = BYTE1(i);
	NRF24L01_TXDATA[16] = BYTE0(i);
	NRF24L01_TXDATA[17] = BYTE1(i);
	NRF24L01_TXDATA[18] = BYTE0(i);
	NRF24L01_TXDATA[19] = BYTE1(i);
	NRF24L01_TXDATA[20] = BYTE0(i);
        NRF24L01_TXDATA[21] = BYTE1(i);
	NRF24L01_TXDATA[22] = BYTE0(i);
        NRF24L01_TXDATA[23] = BYTE1(i);
        NRF24L01_TXDATA[24] = BYTE0(i);
        NRF24L01_TXDATA[25] = BYTE0(i);
        NRF24L01_TXDATA[26] = BYTE0(i);
        NRF24L01_TXDATA[27] = BYTE0(i);
        NRF24L01_TXDATA[28] = BYTE0(i);
        NRF24L01_TXDATA[29] = BYTE0(i);
        NRF24L01_TXDATA[30] = BYTE0(i);

	uint8 sum = 0;
	for(i=0;i<31;i++)
		sum += NRF24L01_TXDATA[i];
	NRF24L01_TXDATA[31] = sum;
	if(nrf_tx(NRF24L01_TXDATA,DATA_PACKET) == 1)
		return 1;
	else 
                return 0;
}