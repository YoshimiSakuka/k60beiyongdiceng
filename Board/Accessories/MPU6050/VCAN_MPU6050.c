...
/**************************实现函数********************************************
*函数原型:		void MPU6050_setFullScaleGyroscopeRange(uint8_t range)
...
*功　　能:	    读取 MPU6050的当前测量值 获得姿态
*******************************************************************************/
void MPU6050_getMotion6(int16_t* ax, int16_t* ay, int16_t* az, int16_t* gx, int16_t* gy, int16_t* gz) {
  
	if(MPU6050_is_DRY())            //检查MPU6050的中断引脚，测试是否完成转换
        {                                   
          IICreadBytes(devAddr, MPU6050_RA_ACCEL_XOUT_H, 14, buffer);//读取指定设备 指定寄存器的 length个值
